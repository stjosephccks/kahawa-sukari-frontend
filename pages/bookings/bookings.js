...
function Bookings() {
    ...
    // Assign random colors to venues (memoized to keep consistent)
    const venuesWithColors = useMemo(() => {
        return venues.map((venue, index) => ({
            ...venue,
            color: venue.color || colorPalette[index % colorPalette.length]
        }));
    }, [venues]);
    ...
    const venueForSelected = selectedBooking ? venuesWithColors.find(v => v.id === selectedBooking.venue_id) : null;
    ...
    return (
        <Layout>
            ...
            {/* Venue Key with Colors */}
            <div className="mb-6 p-6 border rounded-xl bg-gradient-to-r from-gray-50 to-blue-50 shadow-md">
                <h2 className="text-xl font-bold mb-4 text-gray-800 flex items-center gap-2">
                    <span>🎨</span> Venue Key
                </h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                    {venuesWithColors.map(venue => (
                        <div key={venue.id} className="flex items-center gap-2 p-2 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow">
                            <span className={`w-5 h-5 rounded-full ${venue.color} shadow-md flex-shrink-0`}></span>
                            <span className="text-sm font-medium text-gray-700 truncate" title={venue.name}>
                                {venue.name}
                            </span>
                        </div>
                    ))}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-all duration-300" onClick={() => setShowModal(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl p-4 sm:p-6 max-w-full w-[95vw] sm:max-w-md relative animate-fadeIn" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex items-center gap-2 mb-2">
                            <span className={`inline-block w-4 h-4 rounded-full ${venueForSelected?.color || 'bg-gray-400'} shadow-md`}></span>
                        ...
                        {venueForSelected && (
                            <div className="mt-4 p-4 rounded-xl border bg-gradient-to-br from-blue-50 to-indigo-50 shadow-sm">
                                <div className="flex items-center gap-2 mb-2">
                                    <span className={`inline-block w-4 h-4 rounded-full ${venueForSelected.color} shadow-md`}></span>
                                    ...
                                    <div key={idx} className={`min-h-[70px] sm:min-h-[90px] rounded-xl border border-blue-100 bg-white flex flex-col items-start p-1 shadow-sm transition-all duration-200 hover:shadow-md relative group ${date && date.toDateString() === today.toDateString() ? 'ring-2 ring-blue-400' : ''}`}>
                                        ...
                                        <div className="flex flex-col gap-1 w-full">
                                            {bookingsForDay.length === 0 && date ? (
                                                ...
                                                const venue = venuesWithColors.find(v => v.id === booking.venue_id);
                                                const color = venue?.color || 'bg-gray-500';
                                                return (
                                                    <button
                                                        key={booking.id}
                                                        className={`rounded-lg px-2 py-1 text-xs text-white font-medium truncate shadow-md border border-white ${color} hover:scale-105 hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-400 transition-all duration-150 w-full text-left`}
                                                    ...
                                                    >
                                                        <div className="flex flex-col items-start text-left">
                                                            <span className="truncate w-full font-semibold" title={booking.title}>
                                                                {booking.title || 'Booking'}
                                                            </span>
                                                            <span className="text-[10px] font-normal text-white/90 flex items-center gap-1">
